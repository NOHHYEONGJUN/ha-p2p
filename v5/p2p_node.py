        ...
        try:
            ...
            if votes:
                ...
                if winner_id == self.id:
                    ...
                    self.etcd_client.put(cluster_key, cluster_info)
                    logger.info(f"Updated cluster info with new manager: {self.id}")
                    ...
                    # 클러스터 정보 확인 및 갱신 필요 여부 확인
                    current_time = time.time()
                    cluster_key = f"{CLUSTER_INFO_PREFIX}/{self.cluster_id}/info"
                    cluster_info = self.etcd_client.get(cluster_key)
                    
                    # 클러스터 정보가 이전 관리자 정보를 갖고 있다면 돕기
                    if cluster_info and cluster_info.get("manager_id") != winner_id:
                        outdated_manager = cluster_info.get("manager_id")
                        last_updated = cluster_info.get("last_updated", 0)
                        if current_time - last_updated > BACKUP_ACTIVATION_TIMEOUT:
                            logger.info(f"Helping update cluster info from {outdated_manager} to {winner_id}")
                            updated_info = cluster_info.copy()
                            updated_info["manager_id"] = winner_id
                            updated_info["is_primary"] = True
                            updated_info["election_id"] = election_id
                            updated_info["elected_at"] = current_time
                            updated_info["last_updated"] = current_time
                            self.etcd_client.put(cluster_key, updated_info)
            else:
        ...
        try:
            ...
            try:
                ...
                for key, manager_data in temp_manager_results:
                    temp_manager_id = manager_data.get("node_id")
                    temp_manager_cluster = manager_data.get("cluster_id")
                    if temp_manager_id and temp_manager_cluster:
                        logger.info(f"임시 관리자 {temp_manager_id} 비활성화 요청")
                        deactivation_key = f"{BACKUP_INFO_PREFIX}/{self.cluster_id}/deactivation"
                        deactivation_data = {
                            "timestamp": time.time(),
                            "new_manager_id": self.id,
                            "status": "REQUESTED"
                        }
                        self.etcd_client.put(deactivation_key, deactivation_data)
            except Exception as e:
                logger.error(f"임시 관리자 비활성화 요청 중 오류: {e}")
                
            # 클러스터 정보 업데이트 - 새 관리자 등록 (이전 정보 완전 대체)
            time.sleep(0.5)  # 약간의 지연으로 경쟁 상태 방지
            cluster_key = f"{CLUSTER_INFO_PREFIX}/{self.cluster_id}/info"
            cluster_info = {
                "cluster_id": self.cluster_id,
                "manager_id": self.id,
            ...
            # ETCD 클라이언트에 직접 API 호출로 강제 PUT
            try:
                self.etcd_client.primary_client.delete(cluster_key)
                self.etcd_client.put(cluster_key, cluster_info)
                logger.info(f"클러스터 정보 업데이트 완료: 새 관리자 {self.id}")
            except Exception as e:
                logger.error(f"클러스터 정보 업데이트 중 오류: {e}")
